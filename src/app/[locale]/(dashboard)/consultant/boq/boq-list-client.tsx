...
import { publishBOQ, deleteBOQ } from "@/server/actions/boq.actions";
...
export function BOQListClient({ boqs }: { boqs: BOQ[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">{t("boq.myBoqs")}</h1>
          <p className="text-muted-foreground text-sm mt-1">
            {boqs.length} {t("boq.title").toLowerCase()}
          </p>
        </div>
        <Button onClick={() => router.push("/consultant/boq/create" as any)}>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <CardTitle
                      ...
                      onClick={() => router.push(`/consultant/boq/${boq.id}` as any)}
                    >
                      {boq.title}
                    </CardTitle>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {boq.status === "PUBLISHED" && (
                    <Button
                      ...
                      onClick={() => router.push(`/consultant/boq/${boq.id}` as any)}
                    >
                      {t("boq.bids")} ({boq.materialBids.length})
                    </Button>
                  )}
                </div>
              </CardContent>
