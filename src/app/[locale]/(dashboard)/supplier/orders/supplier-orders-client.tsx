...
type Order = {
  ...
  currency: string;
  createdAt: Date;
  boq: { title: string } | null;
  bid: { totalPrice: number | string };
...
export function SupplierOrdersClient({ orders }: { orders: Order[] }) {
            ...
            <Card key={order.id} className="hover:shadow-md transition-shadow">
              ...
              <CardContent className="pt-0 flex items-center justify-between gap-4">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => router.push(`/supplier/orders/${order.id}` as any)}
                >
                  {t("common.view")}
